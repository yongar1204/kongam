...
<body>
    ...
    <button type="button" class="testbtn">testbtn</button>
</body>
...
$(document).ready(function (){
    ...
    $("input[type=file]").change(function () {
        ...
        /* var infiles = this.files;
        for(var i=0; i<infiles.length; i++){
        	fileList2.push(infiles[i]);
        } */
	...
	$('.boardSubmit').click(function () {
		...
		var files = $('#addAttachment').prop('files');
		/* for(var i=0; i<files.length; i++){
			ajaxData.append('files', files[i])
		} */
		for(var i=0; i<fileList2.length; i++){
		ajaxData.append('files', fileList2[i])
		}
		var text = {};
		let formDataAr = $('#board-form').serializeArray();
	    let len = formDataAr.length;
	    for (let i = 0; i < len; i++) {
	        text[formDataAr[i].name] = formDataAr[i].value;
	    }
		ajaxData.append('text', new Blob([JSON.stringify(text)] , {type: "application/json"}));
		$.ajax({
			url : '/board/api/test.do',
			data : ajaxData,
			type : 'POST',
			contentType: false,
			processData : false,
