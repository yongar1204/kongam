$(function(){
	...
	$(".tabNav a").click(function(e) {
		...
		var tPin = $(this).attr('href'),
			navbarHeight = $('body > header').outerHeight(),
			i = $(this).index();
		...
		if(i == 0){
			$('html,body').stop().animate({
				scrollTop: $(tPin).offset().top  - navbarHeight
			},500);
		}else{
			$('html,body').stop().animate({
				scrollTop: $(tPin).offset().top  - (navbarHeight / 2)
			},500);
		}
	...
	$(window).scroll(function() {
		...
		if(scroll + $(window).innerHeight()  == $(document).height()  ) {
